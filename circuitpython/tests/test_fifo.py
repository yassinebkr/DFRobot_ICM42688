...
def test_enable_fifo_default(icm, mock_i2c_device):
    """Test enabling FIFO with default parameters."""
    icm.enable_fifo()

    # Check FIFO_CONFIG1 register
    fifo_config1 = get_register_value(mock_i2c_device, 0x5F)
    ...
    fifo_config = get_register_value(mock_i2c_device, 0x16)
    assert fifo_config == reg.FIFO_MODE_STREAM


def test_enable_fifo_custom(icm, mock_i2c_device):
...
def test_disable_fifo(icm, mock_i2c_device):
    """Test disabling FIFO."""
    # Enable first
    icm.enable_fifo()
...
def test_flush_fifo(icm, mock_i2c_device):
    """Test flushing FIFO."""
    # Simulate some data in FIFO
    mock_i2c_device.register_map[0x2E] = 0x00  # FIFO_COUNTH
    mock_i2c_device.register_map[0x2F] = 0x20  # FIFO_COUNTL = 32 bytes
...
def test_fifo_count(icm, mock_i2c_device):
    """Test reading FIFO count."""
    # Simulate FIFO with 48 bytes (3 packets)
    mock_i2c_device.register_map[0x2E] = 0x00  # FIFO_COUNTH
    mock_i2c_device.register_map[0x2F] = 0x30  # FIFO_COUNTL = 48

    count = icm.fifo_count
    assert count == 48


def test_fifo_count_large(icm, mock_i2c_device):
    """Test reading FIFO count with large value."""
    # Simulate FIFO with 512 bytes
    mock_i2c_device.register_map[0x2E] = 0x02  # FIFO_COUNTH = 2
    mock_i2c_device.register_map[0x2F] = 0x00  # FIFO_COUNTL = 0

    count = icm.fifo_count
    assert count == 512


def test_read_fifo_packet(icm, mock_i2c_device):
    ...
    for i, byte in enumerate(fifo_data):
        mock_i2c_device.register_map[0x30 + i] = byte

    # Simulate some bytes in FIFO
    mock_i2c_device.register_map[0x2E] = 0x00
    mock_i2c_device.register_map[0x2F] = 0x10  # 16 bytes
...
def test_fifo_data_units(icm, mock_i2c_device):
...
def test_fifo_multiple_packets(icm, mock_i2c_device):
    ...
    mock_i2c_device.register_map[0x2E] = 0x00
    mock_i2c_device.register_map[0x2F] = 0x30  # 48 bytes = 3 packets
...
def test_fifo_after_reset(icm, mock_i2c_device):
