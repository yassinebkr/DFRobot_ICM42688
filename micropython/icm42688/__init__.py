...
"""
MicroPython driver for the ICM42688 6-axis IMU sensor.

This library provides a MicroPython interface for the TDK InvenSense ICM42688
...
MicroPython API Differences from CircuitPython
-----------------------------------------------

**Import Changes:**
- CircuitPython: `import board; import busio`
- MicroPython: `from machine import I2C, SPI, Pin`

**I2C Initialization:**
- CircuitPython: `i2c = busio.I2C(board.SCL, board.SDA)`
- MicroPython: `i2c = I2C(0, scl=Pin(22), sda=Pin(21), freq=400000)`

**SPI Initialization:**
- CircuitPython: `spi = busio.SPI(board.SCK, MOSI=board.MOSI, MISO=board.MISO)`
- MicroPython: `spi = SPI(1, baudrate=10000000, sck=Pin(18), mosi=Pin(23), miso=Pin(19))`

**Pin Numbers:**
- CircuitPython: `board.GP1` (symbolic names)
- MicroPython: Numeric pins (e.g., `Pin(5)`)

**Error Types:**
- I2C/SPI communication errors raise `OSError` in MicroPython
- CircuitPython may raise `RuntimeError` instead

All other APIs remain identical for easy migration.
"""
...
class ICM42688:
    ...
    :param cs: Chip select pin (required for SPI, ignored for I2C)
                Must be a machine.Pin object configured as OUTPUT
    ...
    **Example: I2C (ESP32)**
    ...
    .. code-block:: python
        ...
        # ESP32 WROOM: I2C0 on GPIO21 (SDA) and GPIO22 (SCL)
    ...
    **Example: SPI (ESP32)**
    ...
    .. code-block:: python
        ...
        # ESP32: SPI on GPIO18 (SCK), GPIO23 (MOSI), GPIO19 (MISO), GPIO5 (CS)
    ...
    def __init__(
        ...
        # Auto-detect I2C vs SPI based on object type
        if isinstance(i2c_or_spi, I2C):
            # I2C mode
            self._i2c = i2c_or_spi
            self._use_spi = False
        elif isinstance(i2c_or_spi, SPI):
            # SPI mode
            if cs is None:
                raise ValueError("Chip select (cs) pin required for SPI")
            if not isinstance(cs, Pin):
                raise ValueError("CS must be a machine.Pin object")

            self._spi = i2c_or_spi
            self._cs = cs
            self._cs.value(1)  # CS idle high
            self._use_spi = True
        else:
            raise ValueError("Expected machine.I2C or machine.SPI object")
        ...
        # This is critical for real-time aerospace applications to avoid GC pauses
    ...
    def _init_sensor(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"ICM42688 initialization failed: {e}") from e

    # ========================================================================
    # Low-level register access methods (MicroPython I2C/SPI primitives)
    # ========================================================================
    ...
    def _set_bank(self, bank: int) -> None:
        ...
        if self._current_bank != bank:
            ...
            try:
                if self._use_spi:
                    # SPI write: register address (bit 7=0 for write) + data
                    self._cs.value(0)
                    self._spi.write(bytes([reg.REG_BANK_SEL & 0x7F, bank]))
                    self._cs.value(1)
                else:
                    # I2C write: register address + data
                    self._i2c.writeto_mem(self._address, reg.REG_BANK_SEL, bytes([bank]))

                self._current_bank = bank
            except OSError as e:
                raise RuntimeError(f"Failed to switch to bank {bank}: {e}") from e
    ...
    def _read_register_byte(self, register: int) -> int:
        ...
        try:
            if self._use_spi:
                # SPI read: set bit 7 high for read operation
                self._cs.value(0)
                ...
                # I2C read: use readfrom_mem
                result = self._i2c.readfrom_mem(self._address, register, 1)
                return result[0]
        except OSError as e:
            raise RuntimeError(f"Failed to read register 0x{register:02X}: {e}") from e

    def _read_register_bytes(self, register: int, length: int, buf = None):
        """
        ...
        try:
            if self._use_spi:
                ...
                self._cs.value(0)
                self._spi.write(bytes([register | 0x80]))
                self._spi.readinto(buf, length)
                self._cs.value(1)
            else:
                # I2C: read from consecutive registers
                result = self._i2c.readfrom_mem(self._address, register, length)
                buf[:length] = result

            return buf
        except OSError as e:
            raise RuntimeError(f"Failed to read {length} bytes from register 0x{register:02X}: {e}") from e
    ...
    def _write_register_byte(self, register: int, value: int) -> None:
        ...
        try:
            if self._use_spi:
                # SPI write: register address (bit 7=0) + data
                self._cs.value(0)
                self._spi.write(bytes([register & 0x7F, value]))
                self._cs.value(1)
            else:
                # I2C write: use writeto_mem
                self._i2c.writeto_mem(self._address, register, bytes([value]))
        except OSError as e:
            raise RuntimeError(f"Failed to write 0x{value:02X} to register 0x{register:02X}: {e}") from e
    ...
    def _write_register_bytes(self, register: int, data: bytes) -> None:
        ...
        try:
            if self._use_spi:
                ...
                self._cs.value(0)
                self._spi.write(bytes([register & 0x7F]) + data)
                self._cs.value(1)
            else:
                # I2C: write to consecutive registers
                self._i2c.writeto_mem(self._address, register, data)
        except OSError as e:
            raise RuntimeError(f"Failed to write {len(data)} bytes to register 0x{register:02X}: {e}") from e
    ...
    def _read_sensor_data(self):
        ...
        # Read all 14 bytes in one transaction for efficiency
    ...
    # ========================================================================
    # Public properties for sensor readings (API compatible with CircuitPython)
    # ========================================================================
